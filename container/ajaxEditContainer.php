<?php
	...
	if(isset($_POST)){
        ...
        if( $supplyorderid == '-1' || strlen($containerref) < 3 || strlen($description) < 3 || $expenses < 0 || $buyingprice < 0 || $hasdeficit == '-1' || strlen($deficitdesc) < 3 || $hasoutstanding == '-1' || strlen($outstandingdesc) < 3 || strlen($shippingdate) < 10 || strlen($seaportarrivaldate) < 10 || strlen($clearancedate) < 10 || strlen($datemovedtowarehouse) < 10 ){
            $err = 'Please fill in all fields';
        }else{
            $response = addNewContainerRecord($supplyorderid, $description, $hasdeficit, $deficitdesc, $hasoutstanding, $outstandingdesc, $containerref, $shippingdate, $seaportarrivaldate, $clearancedate, $datemovedtowarehouse, $buyingprice, $expenses, true, $containerid);
            //echo $response; exit;
            if($response == 'success'){
                $err = '1';
            }else{
